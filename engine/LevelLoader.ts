import type { Arrow, LevelDefinition } from "@/types/game";
import { getOccupiedCells, hasSelfIntersection } from "@/engine/ArrowEngine";
...
export async function loadLevelById(id: string): Promise<LevelDefinition> {
  const mod = (await import(`@/levels/${id}.json`)) as { default: LevelDefinition };
  return mod.default;
}
